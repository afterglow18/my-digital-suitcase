...
function SearchItemRow({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-3 p-2 border-2 border-black rounded-xl bg-white
                 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]
                 active:translate-y-0.5 active:translate-x-0.5 active:shadow-none transition-all"
    >
      <div className="w-12 h-12 border-2 border-black rounded-lg overflow-hidden flex-shrink-0"
      ...
      <div className="flex-1 min-w-0 text-left">
        ...
        <p className="text-[10px] text-black/40 font-medium uppercase tracking-wide">{item.category}</p>
      </div>
    </button>
  );
}
...
function SearchGroupRow({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-3 p-3 border-2 border-black rounded-xl bg-primary
    ...
    >
      <div className="flex gap-1 flex-shrink-0">
        {Array.from({ length: 3 }).map((_, i) => {
          const m = thumbs[i];
          return (
            <div key={i} className="w-10 h-10 border-2 border-black overflow-hidden rounded"
                 style={{ background: "#F5EDD8" }}>
              {m?.imageObjectPath ? (
                <img src={getImageUrl(m.imageObjectPath)!} alt={m.name}
                     className="w-full h-full object-contain" />
              ) : (
                <div className="w-full h-full flex items-center justify-center">
...
export default function SavedPage() {
  ...
  const [searchDetailsItem, setSearchDetailsItem] = useState<ClothingItem | null>(null);
  ...
  // When a group is tapped in search results, clear search and scroll to that outfit
  const outfitRefs = useRef<Record<number, HTMLDivElement | null>>({});
  const handleGroupResultClick = (outfitId: number) => {
    setSearchQuery("");
    setTimeout(() => {
      outfitRefs.current[outfitId]?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 100);
  };

  return (
    <div ref={scrollRef} className="min-h-full flex flex-col pt-8 px-4 pb-8 bg-secondary/10 relative">
      ...
      <div className="mb-5 relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-9 py-3 border-2 border-black rounded-xl text-sm font-medium
        ...
        <div className="flex flex-col gap-5">
          {searchFetching ? (
            <div className="flex flex-col gap-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-16 bg-muted animate-pulse border-2 border-black rounded-xl" />
              ))}
            </div>
          ) : (
            <>
                  ...
                  <div className="flex flex-col gap-2">
                    {searchResults!.items.map(({ item }) => (
                      <SearchItemRow
                        key={item.id}
                        item={item}
                        onClick={() => setSearchDetailsItem(item)}
                      />
                    ))}
                  </div>
                </section>
              ...
              {(searchResults?.groups.length ?? 0) > 0 && (
                <section>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-black/40 mb-2">
                    Looks — {searchResults!.groups.length}
                  </p>
                  <div className="flex flex-col gap-2">
                    {searchResults!.groups.map(({ outfit }) => (
                      <SearchGroupRow
                        key={outfit.id}
                        outfit={outfit}
                        onClick={() => handleGroupResultClick(outfit.id)}
                      />
                    ))}
                  </div>
                </section>
              ...
              {(searchResults?.items.length ?? 0) === 0 &&
               (searchResults?.groups.length ?? 0) === 0 && (
                <div className="flex flex-col items-center justify-center text-center py-12 gap-2">
                  ...
                  <p className="text-sm font-bold text-black/30 uppercase tracking-wide">
      ...
      {/* Item details — from Lookbook card taps, no Add to Lookbook button */}
      <AnimatePresence>
        {detailsItem && (
          <ItemDetailsSheet
            key={detailsItem.id}
      ...
      {/* Item details — from search results, Add to Lookbook shown */}
      <AnimatePresence>
        {searchDetailsItem && (
          <ItemDetailsSheet
            key={`search-${searchDetailsItem.id}`}
            item={searchDetailsItem}
            onClose={() => setSearchDetailsItem(null)}
            showAddToLookbook={true}
          />
        )}
      </AnimatePresence>
    </div>
